...
import { redisRemoveAuthToken, redisPutAuthToken } from '../utils/redisUtil';
...
const getAuth = async (req: Request, res: Response) => {
  ...
  await redisPutAuthToken(req.cookies.authToken, req.body.user.id, req.body.user.name, req.body.user.email);
  res.status(StatusCodes.OK).json(authUserDetails);
}

const removeAuth = async (req: Request, res: Response) => {
  await redisRemoveAuthToken(req.cookies.authToken);
...
const signin = async (req: Request, res: Response) => {
  ...
  await redisPutAuthToken(token, user.id, user.name as string, user.email);
